...
def set_DASSAULT_license(env, version):
    ...
    if platform.system() == "Darwin" :
        license_file_name=license_file_prefix + '-' + version + '-' + linux_dist + '.dylib'
...
def set_env_build(env, prereq_dir, version):
    ...
    if platform.system() == "Windows" :
        ...
        libdir = "macOS_arm64" # need review
        env.prepend('PATH', os.path.join(prereq_dir, 'bin', libdir))
        env.prepend('DYLD_LIBRARY_PATH', os.path.join(prereq_dir, 'lib', libdir))
