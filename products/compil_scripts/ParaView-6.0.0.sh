...
CMAKE_OPTIONS+=" -DPARAVIEW_VERSIONED_INSTALL=OFF"
### OpenGL settings
CMAKE_OPTIONS+=" -DOpenGL_GL_PREFERENCE:STRING=LEGACY"

case $LINUX_DISTRIBUTION in
    FD4*)
        CMAKE_OPTIONS+=" -DVTK_MODULE_USE_EXTERNAL_ParaView_protobuf:BOOL=ON"
        ;;
    CO10)
        CMAKE_OPTIONS+=" -DVTK_MODULE_USE_EXTERNAL_ParaView_protobuf:BOOL=ON"
        ;;
...
### CGNS
CMAKE_OPTIONS+=" -DVTK_MODULE_ENABLE_VTK_cgns=YES"
CMAKE_OPTIONS+=" -DVTK_MODULE_USE_EXTERNAL_VTK_cgns=YES"
...
# GEOVIS
CMAKE_OPTIONS+=" -DPARAVIEW_ENABLE_GEOVIS:BOOL=ON"

# ONNX
if [ "${ONNX_ROOT_DIR}" != "" ]; then
    echo "WARNING: switch ON the ONNX plugin!"
    CMAKE_OPTIONS+=" -DPARAVIEW_PLUGIN_ENABLE_ONNXPlugin:BOOL=ON"
else
    echo "WARNING: switch OFF the ONNX plugin!"
    CMAKE_OPTIONS+=" -DPARAVIEW_PLUGIN_ENABLE_ONNXPlugin:BOOL=OFF"
fi
